...
def tillampa_valdagskorrigering(snitt: pd.Series, aktivera: bool | None = None) -> pd.Series:
    ...
    Avstängd som standard: ingen norsk skevhet är skattad än, och
    data/valdagskorrigering.csv finns inte. Slås den på utan att filen fyllts
    med norska värden blir korrigeringen tom och snittet oförändrat.
    """
    if aktivera is None:
...
# --- Mandatfördelning --------------------------------------------------------
# Fördelningen ligger i mandat.py och distriktsmodell.py. Båda är validerade
# mot faktiska val: mandat.fordela reproducerar 2021 och 2025 exakt, och
# distriktsmodellen träffar inom sex mandat av 169 ur stickprov.
#
# Den svenska modellens genväg, att approximera riket som en valkrets, går
# inte att använda här. Fyraprocentsspärren gäller bara utjämningsmandaten, så
# ett parti under spärren får mandat uteslutande genom distriktsstyrka.
# Venstres tre mandat 2025 följer inte av riksandelen 3,69 procent.


def fordela_mandat(roster: dict[str, float],
                   platser: int = cfg.MANDAT_TOTALT) -> dict[str, int]:
    """Fördelar mandat från riksandelar, via distriktsmodellen.

    `platser` finns kvar för att signaturen ska likna den svenska modellens,
    men Norge har alltid 169 mandat och ett annat värde avvisas: ett tal som
    tystnadsvis ignoreras vore värre än ett fel.
    """
    if platser != cfg.MANDAT_TOTALT:
        raise ValueError(
            f"Stortinget har {cfg.MANDAT_TOTALT} mandat, inte {platser}. "
            f"Mandatfördelningen bygger på distriktsstrukturen och kan inte "
            f"räknas om för ett annat totalantal.")
    res = distriktsmodell.prognos(dict(roster))
    return {p: res["mandat"].get(p, 0) for p in roster}


def _fordela_mandat_vektor(roster: np.ndarray) -> np.ndarray:
    """Mandatfördelning för många simuleringar.

    Kan inte vektoriseras som i den svenska modellen: varje simulering kräver
    en fördelning i nitton distrikt plus en nationell utjämning med iterativ
    överfördelningskontroll. Loopen är alltså oundviklig, och kostnaden ligger
    i stället nedtryckt i mandat.sainte_lague och i att distriktsprofilen
    cachas mellan anropen.

    En cache på avrundade röstvektorer prövades och gav noll träffar: med nio
    partier är en vektor avrundad till en decimal så gott som alltid unik.
    """
    ...
    for i in range(n_sim):
        res = distriktsmodell.prognos(
            {p: float(roster[i, j]) for j, p in enumerate(partier)})
        mandat[i] = [res["mandat"].get(p, 0) for p in partier]
    return mandat
...
def simulera(snitt: pd.Series, dagar_kvar: int, n: int | None = None,
    ...
    # Driften mättas i stället för att växa som roten ur tiden rakt igenom.
    # Ett backtest mot de fyra valen 2013 till 2025 visar att felet planar ut
    # kring 4,5 procentenheter efter ungefär två år: partistödet är
    # medelåtervändande, det vandrar inte fritt. Utan mättnaden ger modellen
    # 7,8 procentenheters drift på tre års avstånd, vilket är nästan dubbelt
    # mot vad som faktiskt observerats. Halveringspunkten styrs av
    # drift_mattnad_manader.
    mattnad = cfg.DRIFT_MATTNAD_MANADER
    effektiva_manader = manader_kvar / (1.0 + manader_kvar / mattnad)
    drift_sd = cfg.TREND_DRIFT_SD_PER_MANAD * np.sqrt(effektiva_manader) * 100
...
def sammanfatta(sim: dict) -> pd.DataFrame:
    ...
    for i, p in enumerate(partier):
        ...
        forra_stod = cfg.VALRESULTAT_2025.get(p)
        forra_mandat = cfg.MANDAT_2025.get(p)
        rader.append({
            "parti": p,
            ...
            # Jämförelse mot valet 2025, så att förändringen går att läsa direkt.
...
def blockutfall(sim: dict) -> dict:
    ...
    for block, medlemmar in cfg.BLOCK.items():
        ...
        forra_roster = sum(cfg.VALRESULTAT_2025.get(p, 0.0) for p in medlemmar)
        forra_mandat = sum(cfg.MANDAT_2025.get(p, 0) for p in medlemmar)
        mandat_median = int(np.median(m))
        ut[block] = {
            ...
            "sannolikhet_majoritet": float(np.mean(m >= cfg.MAJORITET)),
    ...
    ut["oavgjort"] = float(np.mean((v < cfg.MAJORITET) & (h < cfg.MAJORITET)))
    return ut
